      ...
      if (data.success && data.data?.summary && data.data?.gameweekData) {
        ...
        const stats: TeamStats[] = data.data.summary.map((item: any) => {
          ...
          return {
            ...
            averagePoints: item.totalPoints && data.data.completedGameweeks 
              ? Math.round(item.totalPoints / data.data.completedGameweeks) : 0,
        ...
        // Build leaderboard entries with gameweek history
        ...
        <div className="container mx-auto px-4 py-6">
          <BlurFade delay={0}>
            <div className="flex items-center justify-between mb-6">
              <div>
                <h1 className="text-2xl font-bold text-[#1A1F16] dark:text-[#FFFCF2]">Team Statistics</h1>
                <p className="text-sm text-[#19297C] dark:text-[#DBC2CF]">Performance analysis</p>
              </div>
          ...
          <BlurFade delay={0.1}>
            <Tabs defaultValue="league">
              <TabsList className="mb-4 bg-white dark:bg-[#1A1F16] border border-[#DBC2CF] dark:border-[#19297C]">
                <TabsTrigger value="league" className="gap-2 data-[state=active]:bg-[#F26430] data-[state=active]:text-white">
                  <Users className="w-4 h-4" /> League
                </TabsTrigger>
                <TabsTrigger value="custom" className="data-[state=active]:bg-[#F26430] data-[state=active]:text-white">
                  Custom Lookup
                </TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="league" className="space-y-4">
                <div className="flex items-center justify-between gap-4">
                  <div className="relative flex-1 max-w-xs">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[#19297C] dark:text-[#DBC2CF]" />
                    <Input
                      placeholder="Search..."
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                      className="pl-9 bg-white dark:bg-[#1A1F16] border-[#DBC2CF] dark:border-[#19297C] text-[#1A1F16] dark:text-[#FFFCF2] placeholder:text-[#19297C] dark:placeholder:text-[#DBC2CF]"
                    />
                  </div>
                  
                  <div className="flex items-center gap-2 bg-white dark:bg-[#1A1F16] border border-[#DBC2CF] dark:border-[#19297C] rounded-md p-1">
                    <Button
                      variant={viewMode === "cards" ? "default" : "ghost"}
                      size="sm"
                      onClick={() => setViewMode("cards")}
                      className={viewMode === "cards" 
                        ? "bg-[#F26430] text-white hover:bg-[#F26430]/90" 
                        : "text-[#19297C] dark:text-[#DBC2CF] hover:text-[#F26430] dark:hover:text-[#028090]"
                      }
                    >
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {filteredTeams.map((team, i) => (
                    <BlurFade key={team.teamId} delay={0.03 * i}>
                      <PlayerCard stats={team} rank={team.leaderboardPosition} />
                    </BlurFade>
                  ))}
                </div>
              ) : (
              ...
              <TabsContent value="custom">
                <Card className="max-w-md bg-white dark:bg-[#1A1F16] border-[#DBC2CF] dark:border-[#19297C]">
                  <CardHeader>
                    <CardTitle className="text-[#1A1F16] dark:text-[#FFFCF2]">Custom Team Lookup</CardTitle>
                    <CardDescription className="text-[#19297C] dark:text-[#DBC2CF]">Enter FPL team ID</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="flex gap-2">
                      <Input
                        ...
                        className="bg-white dark:bg-[#1A1F16] border-[#DBC2CF] dark:border-[#19297C] text-[#1A1F16] dark:text-[#FFFCF2] placeholder:text-[#19297C] dark:placeholder:text-[#DBC2CF]"
                        type="number"
                      />
                      <Button className="bg-[#F26430] text-white hover:bg-[#F26430]/90">
                        Search
                      </Button>
                    </div>
                  </CardContent>
